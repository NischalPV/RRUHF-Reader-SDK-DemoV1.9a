...
namespace RRUHF_Reader_SDK_Demo
{
    public class CRC16
    {
		private static ushort[] Crc16CcittTable = new ushort[]
		{
		...
		public static ushort Calc(byte[] byteBuffer)
		{
			uint num = 65535U;
			for (int i = 1; i < byteBuffer.Length; i++)
			{
				num = (uint)((ushort)((num << 8) ^ (uint)Crc16CcittTable[(int)(((num >> 8) ^ (uint)byteBuffer[i]) & 255U)]));
			}
			return (ushort)(num ^ 0xFFFF);
		}


		public static ushort Calc(byte[] buffer, int index, int size)
        {
